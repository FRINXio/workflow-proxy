...
describe('Task transformers', () => {
  ...
  test('GET task before', () => {
    ...
    return new Promise((resolve) => {
      ...
      transformer(
        mockIdentity('FACEBOOK', ['network-admin'], []),
        mockReq,
        null,
        callback,
      );
      ...
      expect(mockReq.url).toStrictEqual(
        '/api/tasks/poll/FACEBOOK___testTask?domain=FACEBOOK',
      );
    });
  });
  ...
  test('GET task before with additional params', () => {
    ...
    let mockReq = mockRequest(
      ...
      { taskType: 'testTask' },
      { from: 'a@fb.com' },
      { query: 'workerid=abcd&domain=DOM' },
    );
    return new Promise((resolve) => {
      let callback = function () {
        resolve();
      ...
      expect(mockReq.url).toStrictEqual(
        '/api/tasks/poll/FACEBOOK___testTask?workerid=abcd&domain=FACEBOOK',
      );
    });
  });
  ...
  test('GET tasks batch before with additional params', () => {
    ...
    let mockReq = mockRequest(
      ...
      { taskType: 'testTask' },
      { from: 'a@fb.com' },
      { query: 'workerid=abcd&domain=DOM&timeout=10&count=5' },
    );
    return new Promise((resolve) => {
      let callback = function () {
        resolve();
      ...
      expect(mockReq.url).toStrictEqual(
        '/api/tasks/poll/batch/FACEBOOK___testTask?workerid=abcd&count=5&timeout=10&domain=FACEBOOK',
      );
    });
  });
