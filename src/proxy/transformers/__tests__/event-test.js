...
const eventHandlerDefPrefixed = () => JSON.parse(JSON.stringify(eventHandlerDefPrefixedBody));

describe('Event transformers', () => {
  const transformers = events({"proxyTarget": "PROXY_TARGET"});
  test("POST event execution before", () => {
    const eventPostTransformer = findTransformerFx(transformers, "/api/event", "post", "before");
  ...
  test("PUT event execution before", () => {
    const eventPutTransformer = findTransformerFx(transformers, "/api/event", "put", "before");
