...
describe('Workflow def transformers', () => {
  ...
  test('READ workflow before', () => {
    const workflowGetBefore = findTransformerFx(
      transformers,
    ...
    return new Promise((resolve) => {
      ...
      expect(mockReq.url).toStrictEqual(
        '/api/metadata/workflow/FACEBOOK___wf31?version=1.1',
      );
    });
  });
  ...
  test('Delete workflow before', () => {
    ...
    return new Promise((resolve) => {
      ...
      expect(mockReq.url).toStrictEqual(
        '/api/metadata/workflow/FACEBOOK___wf31/1.1',
      );
    });
  });
